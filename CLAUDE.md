...
- **location** (nullable): which of the restaurant's 3 physical venues — `FLOOR` (Étage) / `BAR` (Bar) /
  `COUNTER` (Comptoir). Plain picker, no capacity/budget enforcement by design (deliberately simple for
  now — real-world choice depends on guest count and budget, but nothing validates that yet). Filterable
  on the banquets table view (date range, location, guest-count range) — not added to the Contacts list
  since contacts don't carry these fields themselves.
