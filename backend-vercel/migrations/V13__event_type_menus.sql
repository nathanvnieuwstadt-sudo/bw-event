...
-- wholesale on update (see routes.ts), never referenced directly by a
-- banquet — picking a preset menu just copies its dishes into that
-- banquet's own menu_items, which stay freely editable afterward.
...
ALTER TABLE event_type_menus ENABLE ROW LEVEL SECURITY;
ALTER TABLE event_type_menu_items ENABLE ROW LEVEL SECURITY;
