...
public interface AgentDraftRepository extends JpaRepository<AgentDraft, UUID> {
    ...
    @Query("SELECT d FROM AgentDraft d JOIN FETCH d.emailThread et LEFT JOIN FETCH et.contact LEFT JOIN FETCH d.banquet b LEFT JOIN FETCH b.contact WHERE d.restaurantId = :restaurantId ORDER BY d.createdAt DESC")
    List<AgentDraft> findAllByRestaurantIdOrderByCreatedAtDesc(@Param("restaurantId") UUID restaurantId);

    @Query("SELECT d FROM AgentDraft d JOIN FETCH d.emailThread et LEFT JOIN FETCH et.contact LEFT JOIN FETCH d.banquet b LEFT JOIN FETCH b.contact WHERE d.restaurantId = :restaurantId AND d.status = :status ORDER BY d.createdAt DESC")
