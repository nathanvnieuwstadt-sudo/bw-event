...
function IconChevron({ open }: { open: boolean }) {
  return (
    <svg
      width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"
...
export function BanquetFilterBar({
  ...
  // Filters start collapsed on phones (toggled via the button below) but are
  // always visible from `sm:` up — the mobileOpen state only ever matters
  ...
  return (
    <div>
      <button
        ...
        className="flex min-h-[44px] w-full items-center justify-between gap-2 rounded-md border border-neutral-300 bg-white px-3.5 text-sm font-medium text-neutral-700 transition-colors duration-100 hover:bg-neutral-50 active:scale-[0.99] dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800 sm:hidden"
      >
        <span className="flex items-center gap-2">
          <IconFilter />
          Filtres
          {filtersActive && <span className="h-1.5 w-1.5 rounded-full bg-brand-500" />}
        </span>
        <IconChevron open={mobileOpen} />
      </button>

      <div className={`${mobileOpen ? 'flex' : 'hidden'} mt-3 flex-wrap items-end gap-3 sm:mt-0 sm:flex`}>
